...
try:
    ...
    print("[Error] 未找到 models.TimeMixer 或 utils.timefeatures。")
    print("请确保你使用的是 Time-Series-Library 的标准目录结构。")
    sys.exit(1)
...
class TEPPhysicsLoss(nn.Module):
    ...
    def forward(self, preds_norm, inputs_norm):
        ...
        # --- A. 数据反归一化 ---
        # 兼容处理：如果 col_map 没找到 Key，返回默认索引
...
class TimeMixerConfigs:
    def __init__(self, args):
        # 将 args 中的所有参数复制到 configs 中
        self.__dict__.update(vars(args))
        
        # 强制修正部分参数，防止逻辑冲突
        self.use_future_temporal_feature = 0 
        
        # TimeMixer 初始化时可能需要的额外默认值（防止某些版本报错）
        if not hasattr(self, 'output_attention'): self.output_attention = False
        if not hasattr(self, 'distil'): self.distil = True
...
class PGNN(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, u_future=None, x_mark_dec=None):
        B, T, C = x_enc.shape
        # TimeMixer 需要 dec_inp，通常全0或部分历史
...
def prepare_data(args, logger):
    ...
    logger.info(f"[-] 读取数据: {abs_data_path}")
    ...
    logger.info(f"[-] 特征列顺序: {sorted_cols}")
    logger.info(f"[-] 列索引映射: {col_map}")
    ...
    test_loader = DataLoader(Subset(test_set, range(train_len + val_len, total_len - args.seq_len - args.pred_len + 1)), batch_size=args.batch_size, shuffle=False)
...
def main():
    ...
    # --- 基础数据参数 ---
    parser.add_argument('--task_name', type=str, default='long_term_forecast') # 新增
    ...
    parser.add_argument('--d_layers', type=int, default=1) # 补充 Decoder layer 防止报错
    ...
    # -------------------------------------------------------------------------
    # 新增: 归一化空间下的评估 (Normalized Metrics)
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # 原有: 原始物理空间下的评估 (Original Scale Metrics)
    # -------------------------------------------------------------------------
    ...
    # 同时保存两份结果方便后续分析
    np.savez(os.path.join(exp_dir, 'test_results.npz'), 
