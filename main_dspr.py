...
class PhysicsLoader:
    ...
    def load_adjacency_matrix(adj_path, model_columns, device, logger):
        ...
        if not adj_path or not os.path.exists(adj_path):
            logger.info("[Physics] No physics graph file detected or path is empty; using purely data-driven mode.")
            return torch.zeros((num_vars, num_vars), dtype=torch.float32).to(device)

        logger.info(f"[Physics] Loading physics prior graph: {adj_path}")
        try:
            adj_df = pd.read_csv(adj_path, index_col=0)
        except Exception as e:
            logger.error(f"[Error] Failed to read physics graph: {e}")
            return torch.zeros((num_vars, num_vars), dtype=torch.float32).to(device)
        ...
        logger.info(f"[Physics] Successfully mapped {match_count} physics edges.")
        if adj_tensor.max() > 0:
...
class DSPR(nn.Module):
    ...
    def fuse_with_physical_prior(self, learned_adj):
        """Fuse the learned adjacency with physics prior on masked edges."""
        if self.phys_prior is None:
            return learned_adj
    ...
    def forward(self, x_enc, x_mark_enc, u_future=None, x_mark_dec=None, return_extra=False):
        ...
        # 2) Residual stream
        embedded_list = [
            self.channel_embeddings[c](x_enc[:, :, c:c+1]).unsqueeze(2) for c in range(C)
        ]
...
def prepare_data(args, logger):
    ...
    if args.target not in cols:
        logger.warning(f"Target column {args.target} not found; using the last column as target.")
        args.target = cols[-1]

    feature_cols = [c for c in cols if c != args.target]
    sorted_cols = feature_cols + [args.target]

    logger.info(f"[-] Mode: {args.features}")
    logger.info(f"[-] Number of features: {len(sorted_cols)} | Target: {args.target}")

    # Control variable index (if provided and exists)
    u_idx = 0
    ...
    test_loader = DataLoader(
        Subset(test_set, range(train_len + val_len, total - args.seq_len - args.pred_len + 1)),
        batch_size=args.batch_size, shuffle=False
    )
...
def main():
    ...
    # Scheduler: reduce LR by half if validation loss does not improve for 1 epoch
    ...
    for epoch in range(args.train_epochs):
        ...
        for batch_x, batch_y, mark_x, mark_y, u in train_pbar:
            ...
            # Physics regularization on masked edges (if available)
    ...
    with torch.no_grad():
        for batch_x, batch_y, mark_x, mark_y, u in tqdm(test_loader, desc="Testing"):
            ...
            if d_adj is not None:
                # Take a snapshot: average over batch and time/window dims to get [C, C]
    ...
    logger.info(f"[-] Experiment finished. Results archived to: {exp_dir}")
